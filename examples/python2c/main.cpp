 ...
 * cleanup/free arrays
 */
void cleanup(int32_t *a, int32_t *b, int32_t *c, int32_t *d) {
  free(a);
  free(b);
  free(c);
  free(d);
 ...
 */
void mmult_golden(int32_t a[4],int32_t b[4],int32_t c[4]) {
  printf("golden: \n");
  for(int i=0; i<N; i++) {
    printf("[");
    for(int j=0; j<N; j++) {
 ...
 */
int check(int32_t c[4],int32_t d[4]) {
  int err=0;
  for(int i=0; i<N; i++) {
    for(int j=0; j<N; j++) {
      if(c[i*N+j] != d[i*N+j]) {
