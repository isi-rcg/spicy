...
/* 32 bit ELF emulation code for armelf
...
static void gldarmelf_before_parse (void);
static void gldarmelf_after_parse (void);
static void gldarmelf_after_open (void);
static void gldarmelf_before_allocation (void);
static void gldarmelf_after_allocation (void);
static lang_output_section_statement_type *gldarmelf_place_orphan
  (asection *, const char *, int);

static void
gldarmelf_map_segments (bfd_boolean need_layout)
{
  int tries = 10;
...
static void
gldarmelf_after_allocation (void)
{
  int ret;
  ...
  if (need_laying_out != -1)
    gldarmelf_map_segments (need_laying_out);
}

static void
gldarmelf_finish (void)
{
...
static bfd_boolean
gldarmelf_load_symbols (lang_input_statement_type *entry)
{
  int link_class = 0;
...
static void
gldarmelf_vercheck (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldarmelf_stat_needed (lang_input_statement_type *s)
{
  struct stat st;
  const char *suffix;
  const char *soname;
...
static bfd_boolean
gldarmelf_try_needed (struct dt_needed *needed,
				 int force)
{
  bfd *abfd;
  ...
  if (! force)
      ...
      if (needs != NULL)
	{
	  ...
	  lang_for_each_input_file (gldarmelf_vercheck);
	  if (global_vercheck_failed)
	    {
	      bfd_close (abfd);
  ...
  global_found = NULL;
  lang_for_each_input_file (gldarmelf_stat_needed);
  if (global_found != NULL)
    {
...
static bfd_boolean
gldarmelf_search_needed (const char *path,
				    struct dt_needed *n, int force)
{
  ...
  if (name[0] == '/')
    return gldarmelf_try_needed (n, force);
  ...
  while (1)
      ...
      while ((var = strchr (filename + offset, '$')) != NULL)
	{
      ...
      if (gldarmelf_try_needed (&needed, force))
	return TRUE;
...
static void
gldarmelf_check_needed (lang_input_statement_type *s)
{
  const char *soname;
...
gldarmelf_after_open (void)
{
  ...
  for (l = needed; l != NULL; l = l->next)
	...
	continue;
      ...
      global_needed = l;
      global_found = NULL;
      lang_for_each_input_file (gldarmelf_check_needed);
      if (global_found != NULL
	  && (bfd_elf_get_dyn_lib_class (global_found->the_bfd)
	      & DYN_AS_NEEDED) == 0)
	continue;
      ...
      if (global_found != NULL)
	{
	  ...
	  if (gldarmelf_try_needed (&nn, TRUE))
	    continue;
	}
	 ...
	 their use.  See gldarmelf_vercheck comment.  */
      for (force = 0; force < 2; force++)
	{
	  size_t len;
	  search_dirs_type *search;

	  if (gldarmelf_search_needed (command_line.rpath_link,
						  &n, force))
	    break;
	  len = strlen (l->name);
	  for (search = search_head; search != NULL; search = search->next)
		...
		continue;
	      ...
	      nn.name = filename;
	      if (gldarmelf_try_needed (&nn, force))
		break;
	      free (filename);
	    }
	  if (search != NULL)
...
static void
gldarmelf_find_exp_assignment (etree_type *exp)
{
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldarmelf_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldarmelf_find_exp_assignment (exp->binary.lhs);
      gldarmelf_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldarmelf_find_exp_assignment (exp->trinary.cond);
      gldarmelf_find_exp_assignment (exp->trinary.lhs);
      gldarmelf_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldarmelf_find_exp_assignment (exp->unary.child);
      break;
...
static void
gldarmelf_find_statement_assignment (lang_statement_union_type *s)
{
  if (s->header.type == lang_assignment_statement_enum)
    gldarmelf_find_exp_assignment (s->assignment_statement.exp);
}


/* used by before_allocation and handle_option. */
static void
gldarmelf_append_to_separated_string (char **to, char *op_arg)
{
  if (*to == NULL)
    *to = xstrdup (op_arg);
  else
...
static void
gldarmelf_before_allocation (void)
{
  const char *rpath;
  asection *sinterp;
  bfd *abfd;
  ...
  if (is_elf_hash_table (link_info.hash))
      ...
      if (!bfd_link_relocatable (&link_info))
	{
	 ...
	 referred to by dynamic objects.  */
      lang_for_each_statement (gldarmelf_find_statement_assignment);
    }
	...
	if (audit_libs && *audit_libs != '\0')
	  {
		...
		if (cp != NULL && *cp != '\0')
		  gldarmelf_append_to_separated_string (&depaudit, cp);
...
static bfd_boolean
gldarmelf_open_dynamic_archive
...
static lang_output_section_statement_type *
gldarmelf_place_orphan (asection *s,
				   const char *secname,
				   int constraint)
{
...
static char *
gldarmelf_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/armelf.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/armelf.xr";
  else if (!config.text_read_only)
    return "ldscripts/armelf.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/armelf.xn";
	...
	return "ldscripts/armelf.xdwe";
      else
	return "ldscripts/armelf.xdw";
    }
  else if (bfd_link_pie (&link_info)
	   && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/armelf.xdce";
      else
	return "ldscripts/armelf.xdc";
    }
  else if (bfd_link_pie (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/armelf.xde";
      else
	return "ldscripts/armelf.xd";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/armelf.xswe";
      else
	return "ldscripts/armelf.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/armelf.xsce";
      else
	return "ldscripts/armelf.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/armelf.xse";
      else
	return "ldscripts/armelf.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/armelf.xwe";
      else
	return "ldscripts/armelf.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/armelf.xce";
      else
	return "ldscripts/armelf.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/armelf.xe";
      else
	return "ldscripts/armelf.x";
    }
}
...
static bfd_boolean
gldarmelf_handle_option (int optc)
{
  switch (optc)
    {
    default:
	...
	einfo (_("%F%P: invalid --compress-debug-sections option: `%s'\n"),
      ...
      break;
    case OPTION_AUDIT:
	gldarmelf_append_to_separated_string (&audit, optarg);
	break;

    case 'P':
	gldarmelf_append_to_separated_string (&depaudit, optarg);
	break;
...
static void
gldarmelf_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_armelf_emulation =
{
  gldarmelf_before_parse,
  syslib_default,
  hll_default,
  gldarmelf_after_parse,
  gldarmelf_after_open,
  after_check_relocs_default,
  gldarmelf_after_allocation,
  set_output_arch_default,
  ldemul_default_target,
  arm_elf_before_allocation,
  gldarmelf_get_script,
  "armelf",
  "elf32-littlearm",
  gldarmelf_finish,
  arm_elf_create_output_section_statements,
  gldarmelf_open_dynamic_archive,
  gldarmelf_place_orphan,
  gldarmelf_set_symbols,
  NULL,
  gldarmelf_add_options,
  gldarmelf_handle_option,
  NULL,
  gldarmelf_list_options,
  gldarmelf_load_symbols,
  NULL,
  NULL,
  NULL
};
