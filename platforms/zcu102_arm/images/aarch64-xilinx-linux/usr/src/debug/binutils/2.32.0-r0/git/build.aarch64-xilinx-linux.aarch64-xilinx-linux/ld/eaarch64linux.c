...
/* 64 bit ELF emulation code for aarch64linux
...
static void gldaarch64linux_before_parse (void);
static void gldaarch64linux_after_parse (void);
static void gldaarch64linux_after_open (void);
static void gldaarch64linux_before_allocation (void);
static void gldaarch64linux_after_allocation (void);
static lang_output_section_statement_type *gldaarch64linux_place_orphan
...
static void
gldaarch64linux_map_segments (bfd_boolean need_layout)
{
  int tries = 10;
...
static void
gldaarch64linux_before_parse (void)
{
...
aarch64_elf_before_allocation (void)
{
  ...
  gldaarch64linux_before_allocation ();
}
...
gldaarch64_layout_sections_again (void)
{
  /* If we have changed sizes of the stub sections, then we need
     ...
     add even more stubs.  */
  gldaarch64linux_map_segments (TRUE);
  need_laying_out = -1;
}
...
static void
gldaarch64linux_after_allocation (void)
{
  int ret;
  ...
  if (need_laying_out != -1)
    gldaarch64linux_map_segments (need_laying_out);
}

static void
gldaarch64linux_finish (void)
{
...
static void
gldaarch64linux_after_parse (void)
{
...
static bfd_boolean
gldaarch64linux_load_symbols (lang_input_statement_type *entry)
{
  int link_class = 0;
...
static void
gldaarch64linux_vercheck (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldaarch64linux_stat_needed (lang_input_statement_type *s)
{
  struct stat st;
  const char *suffix;
  const char *soname;
...
static bfd_boolean
gldaarch64linux_try_needed (struct dt_needed *needed,
				 int force)
{
  bfd *abfd;
  ...
  if (! force)
      ...
      if (needs != NULL)
	{
	  ...
	  lang_for_each_input_file (gldaarch64linux_vercheck);
	  if (global_vercheck_failed)
	    {
	      bfd_close (abfd);
  ...
  global_found = NULL;
  lang_for_each_input_file (gldaarch64linux_stat_needed);
  if (global_found != NULL)
    {
...
static bfd_boolean
gldaarch64linux_search_needed (const char *path,
				    struct dt_needed *n, int force)
{
  ...
  if (name[0] == '/')
    return gldaarch64linux_try_needed (n, force);
  ...
  while (1)
      ...
      while ((var = strchr (filename + offset, '$')) != NULL)
	{
      ...
      if (gldaarch64linux_try_needed (&needed, force))
	return TRUE;
...
static const char *
gldaarch64linux_add_sysroot (const char *path)
{
  size_t len, extra;
  const char *p;
  char *ret, *q;
...
struct gldaarch64linux_ld_so_conf
{
  char *path;
  size_t len, alloc;
};
...
static bfd_boolean
gldaarch64linux_parse_ld_so_conf
     (struct gldaarch64linux_ld_so_conf *info, const char *filename);

static void
gldaarch64linux_parse_ld_so_conf_include
     (struct gldaarch64linux_ld_so_conf *info, const char *filename,
      const char *pattern)
{
  char *newp = NULL;
  ...
  if (glob (pattern, 0, NULL, &gl) == 0)
	...
	gldaarch64linux_parse_ld_so_conf (info, gl.gl_pathv[i]);
      globfree (&gl);
    }
#else
  /* If we do not have glob, treat the pattern as a literal filename.  */
  gldaarch64linux_parse_ld_so_conf (info, pattern);
#endif
      ...
      if (CONST_STRNEQ (p, "include") && (p[7] == ' ' || p[7] == '\t'))
	{
	  ...
	  do
		...
		++p;
	      ...
	      *p++ = '\0';
	      if (dir[0] != '\0')
		gldaarch64linux_parse_ld_so_conf_include (info, filename,
							       dir);
	    }
	  while (c != '\0');
	}
...
static bfd_boolean
gldaarch64linux_check_ld_so_conf (const struct bfd_link_needed_list *l,
				       int force)
{
  ...
  if (! initialized)
    {
      char *tmppath;
      struct gldaarch64linux_ld_so_conf info;

      info.path = NULL;
      info.len = info.alloc = 0;
      tmppath = concat (ld_sysroot, "/usr/etc/ld.so.conf",
			(const char *) NULL);
      if (!gldaarch64linux_parse_ld_so_conf (&info, tmppath))
	{
	  free (tmppath);
	  tmppath = concat (ld_sysroot, "/etc/ld.so.conf",
			    (const char *) NULL);
	  gldaarch64linux_parse_ld_so_conf (&info, tmppath);
	}
      free (tmppath);

      if (info.path)
	{
	  ld_so_conf = gldaarch64linux_add_sysroot (info.path);
	  free (info.path);
	}
      initialized = TRUE;
    }
  ...
  needed.by = l->by;
  needed.name = l->name;
  return gldaarch64linux_search_needed (ld_so_conf, &needed, force);
}
...
static void
gldaarch64linux_check_needed (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldaarch64linux_after_open (void)
{
  ...
  for (l = needed; l != NULL; l = l->next)
	...
	continue;
      ...
      global_needed = l;
      global_found = NULL;
      lang_for_each_input_file (gldaarch64linux_check_needed);
      if (global_found != NULL
	  && (bfd_elf_get_dyn_lib_class (global_found->the_bfd)
	      & DYN_AS_NEEDED) == 0)
	continue;
      ...
      if (global_found != NULL)
	{
	  ...
	  if (gldaarch64linux_try_needed (&nn, TRUE))
	    continue;
	}
	 ...
	 their use.  See gldaarch64linux_vercheck comment.  */
      for (force = 0; force < 2; force++)
	{
	  ...
	  if (gldaarch64linux_search_needed (command_line.rpath_link,
						  &n, force))
	    break;
	  path = command_line.rpath;
	  if (path)
	    {
	      path = gldaarch64linux_add_sysroot (path);
	      found = gldaarch64linux_search_needed (path, &n, force);
	      free ((char *) path);
	      if (found)
		break;
	    }
	  if (command_line.rpath_link == NULL
	      && command_line.rpath == NULL)
	    {
		  ...
		  path = NULL;
	      if (path
		  && gldaarch64linux_search_needed (path, &n, force))
		break;
	    }
	  ...
	  for (; !found && rp != NULL; rp = rp->next)
	    {
	      path = gldaarch64linux_add_sysroot (rp->name);
	      found = (rp->by == l->by
		       && gldaarch64linux_search_needed (path, &n,
							      force));
	      free ((char *) path);
	    }
	  if (found)
	  ...
	  if (gldaarch64linux_check_ld_so_conf (l, force))
	    break;
	  ...
	  for (search = search_head; search != NULL; search = search->next)
		...
		continue;
	      ...
	      nn.name = filename;
	      if (gldaarch64linux_try_needed (&nn, force))
		break;
	      free (filename);
	    }
	  if (search != NULL)
...
static void
gldaarch64linux_find_exp_assignment (etree_type *exp)
{
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldaarch64linux_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldaarch64linux_find_exp_assignment (exp->binary.lhs);
      gldaarch64linux_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldaarch64linux_find_exp_assignment (exp->trinary.cond);
      gldaarch64linux_find_exp_assignment (exp->trinary.lhs);
      gldaarch64linux_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldaarch64linux_find_exp_assignment (exp->unary.child);
      break;
...
static void
gldaarch64linux_find_statement_assignment (lang_statement_union_type *s)
{
  if (s->header.type == lang_assignment_statement_enum)
    gldaarch64linux_find_exp_assignment (s->assignment_statement.exp);
}


/* used by before_allocation and handle_option. */
static void
gldaarch64linux_append_to_separated_string (char **to, char *op_arg)
{
  if (*to == NULL)
    *to = xstrdup (op_arg);
  else
...
static void
gldaarch64linux_before_allocation (void)
{
  const char *rpath;
  asection *sinterp;
  bfd *abfd;
  ...
  if (is_elf_hash_table (link_info.hash))
      ...
      if (!bfd_link_relocatable (&link_info))
	{
	 ...
	 referred to by dynamic objects.  */
      lang_for_each_statement (gldaarch64linux_find_statement_assignment);
    }
	...
	if (audit_libs && *audit_libs != '\0')
	  {
		...
		if (cp != NULL && *cp != '\0')
		  gldaarch64linux_append_to_separated_string (&depaudit, cp);
  ...
  if (sinterp != NULL)
    {
      sinterp->contents = (unsigned char *) "/lib/ld-linux-aarch64.so.1";
...
static bfd_boolean
gldaarch64linux_open_dynamic_archive
...
static lang_output_section_statement_type *
gldaarch64linux_place_orphan (asection *s,
				   const char *secname,
				   int constraint)
{
...
static char *
gldaarch64linux_get_script (int *isfile)
{
  *isfile = 0;
...
OUTPUT_ARCH(aarch64)\n\
ENTRY(_start)\n\
SEARCH_DIR(\"=/usr/aarch64-xilinx-linux/lib64\"); SEARCH_DIR(\"=/usr/local/lib64\"); SEARCH_DIR(\"=/lib64\"); SEARCH_DIR(\"=/usr/lib64\"); SEARCH_DIR(\"=/usr/aarch64-xilinx-linux/lib\"); SEARCH_DIR(\"=/usr/local/lib\"); SEARCH_DIR(\"=/lib\"); SEARCH_DIR(\"=/usr/lib\");\n\
SECTIONS\n\
{\n\
...
static bfd_boolean
gldaarch64linux_handle_option (int optc)
{
  switch (optc)
    {
    default:
	...
	einfo (_("%F%P: invalid --compress-debug-sections option: `%s'\n"),
      ...
      break;
    case OPTION_AUDIT:
	gldaarch64linux_append_to_separated_string (&audit, optarg);
	break;

    case 'P':
	gldaarch64linux_append_to_separated_string (&depaudit, optarg);
	break;
...
static void
gldaarch64linux_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_aarch64linux_emulation =
{
  gldaarch64linux_before_parse,
  syslib_default,
  hll_default,
  gldaarch64linux_after_parse,
  gldaarch64linux_after_open,
  after_check_relocs_default,
  gldaarch64linux_after_allocation,
  set_output_arch_default,
  ldemul_default_target,
  aarch64_elf_before_allocation,
  gldaarch64linux_get_script,
  "aarch64linux",
  "elf64-littleaarch64",
  gldaarch64linux_finish,
  aarch64_elf_create_output_section_statements,
  gldaarch64linux_open_dynamic_archive,
  gldaarch64linux_place_orphan,
  NULL,
  NULL,
  gldaarch64linux_add_options,
  gldaarch64linux_handle_option,
  NULL,
  gldaarch64linux_list_options,
  gldaarch64linux_load_symbols,
  NULL,
  NULL,
  NULL
};
