...
/* 64 bit ELF emulation code for aarch64elf
...
static void gldaarch64elf_before_parse (void);
static void gldaarch64elf_after_parse (void);
static void gldaarch64elf_after_open (void);
static void gldaarch64elf_before_allocation (void);
static void gldaarch64elf_after_allocation (void);
static lang_output_section_statement_type *gldaarch64elf_place_orphan
  (asection *, const char *, int);

static void
gldaarch64elf_map_segments (bfd_boolean need_layout)
{
  int tries = 10;
...
static void
gldaarch64elf_before_parse (void)
{
...
aarch64_elf_before_allocation (void)
{
  ...
  gldaarch64elf_before_allocation ();
}
...
gldaarch64_layout_sections_again (void)
{
  /* If we have changed sizes of the stub sections, then we need
     ...
     add even more stubs.  */
  gldaarch64elf_map_segments (TRUE);
  need_laying_out = -1;
}
...
static void
gldaarch64elf_after_allocation (void)
{
  int ret;
  ...
  if (need_laying_out != -1)
    gldaarch64elf_map_segments (need_laying_out);
}

static void
gldaarch64elf_finish (void)
{
...
static void
gldaarch64elf_after_parse (void)
{
...
static bfd_boolean
gldaarch64elf_load_symbols (lang_input_statement_type *entry)
{
  int link_class = 0;
...
static void
gldaarch64elf_vercheck (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldaarch64elf_stat_needed (lang_input_statement_type *s)
{
  struct stat st;
  const char *suffix;
  const char *soname;
...
static bfd_boolean
gldaarch64elf_try_needed (struct dt_needed *needed,
				 int force)
{
  bfd *abfd;
  ...
  if (! force)
      ...
      if (needs != NULL)
	{
	  ...
	  lang_for_each_input_file (gldaarch64elf_vercheck);
	  if (global_vercheck_failed)
	    {
	      bfd_close (abfd);
  ...
  global_found = NULL;
  lang_for_each_input_file (gldaarch64elf_stat_needed);
  if (global_found != NULL)
    {
...
static bfd_boolean
gldaarch64elf_search_needed (const char *path,
				    struct dt_needed *n, int force)
{
  ...
  if (name[0] == '/')
    return gldaarch64elf_try_needed (n, force);
  ...
  while (1)
      ...
      while ((var = strchr (filename + offset, '$')) != NULL)
	{
      ...
      if (gldaarch64elf_try_needed (&needed, force))
	return TRUE;
...
static void
gldaarch64elf_check_needed (lang_input_statement_type *s)
{
  const char *soname;
...
gldaarch64elf_after_open (void)
{
  ...
  for (l = needed; l != NULL; l = l->next)
	...
	continue;
      ...
      global_needed = l;
      global_found = NULL;
      lang_for_each_input_file (gldaarch64elf_check_needed);
      if (global_found != NULL
	  && (bfd_elf_get_dyn_lib_class (global_found->the_bfd)
	      & DYN_AS_NEEDED) == 0)
	continue;
      ...
      if (global_found != NULL)
	{
	  ...
	  if (gldaarch64elf_try_needed (&nn, TRUE))
	    continue;
	}
	 ...
	 their use.  See gldaarch64elf_vercheck comment.  */
      for (force = 0; force < 2; force++)
	{
	  size_t len;
	  search_dirs_type *search;

	  if (gldaarch64elf_search_needed (command_line.rpath_link,
						  &n, force))
	    break;
	  len = strlen (l->name);
	  for (search = search_head; search != NULL; search = search->next)
		...
		continue;
	      ...
	      nn.name = filename;
	      if (gldaarch64elf_try_needed (&nn, force))
		break;
	      free (filename);
	    }
	  if (search != NULL)
...
static void
gldaarch64elf_find_exp_assignment (etree_type *exp)
{
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldaarch64elf_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldaarch64elf_find_exp_assignment (exp->binary.lhs);
      gldaarch64elf_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldaarch64elf_find_exp_assignment (exp->trinary.cond);
      gldaarch64elf_find_exp_assignment (exp->trinary.lhs);
      gldaarch64elf_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldaarch64elf_find_exp_assignment (exp->unary.child);
      break;
...
static void
gldaarch64elf_find_statement_assignment (lang_statement_union_type *s)
{
  if (s->header.type == lang_assignment_statement_enum)
    gldaarch64elf_find_exp_assignment (s->assignment_statement.exp);
}


/* used by before_allocation and handle_option. */
static void
gldaarch64elf_append_to_separated_string (char **to, char *op_arg)
{
  if (*to == NULL)
    *to = xstrdup (op_arg);
  else
...
static void
gldaarch64elf_before_allocation (void)
{
  const char *rpath;
  asection *sinterp;
  bfd *abfd;
  ...
  if (is_elf_hash_table (link_info.hash))
      ...
      if (!bfd_link_relocatable (&link_info))
	{
	 ...
	 referred to by dynamic objects.  */
      lang_for_each_statement (gldaarch64elf_find_statement_assignment);
    }
	...
	if (audit_libs && *audit_libs != '\0')
	  {
		...
		if (cp != NULL && *cp != '\0')
		  gldaarch64elf_append_to_separated_string (&depaudit, cp);
...
static bfd_boolean
gldaarch64elf_open_dynamic_archive
...
static lang_output_section_statement_type *
gldaarch64elf_place_orphan (asection *s,
				   const char *secname,
				   int constraint)
{
...
static char *
gldaarch64elf_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/aarch64elf.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/aarch64elf.xr";
  else if (!config.text_read_only)
    return "ldscripts/aarch64elf.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/aarch64elf.xn";
	...
	return "ldscripts/aarch64elf.xdwe";
      else
	return "ldscripts/aarch64elf.xdw";
    }
  else if (bfd_link_pie (&link_info)
	   && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/aarch64elf.xdce";
      else
	return "ldscripts/aarch64elf.xdc";
    }
  else if (bfd_link_pie (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/aarch64elf.xde";
      else
	return "ldscripts/aarch64elf.xd";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/aarch64elf.xswe";
      else
	return "ldscripts/aarch64elf.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/aarch64elf.xsce";
      else
	return "ldscripts/aarch64elf.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/aarch64elf.xse";
      else
	return "ldscripts/aarch64elf.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/aarch64elf.xwe";
      else
	return "ldscripts/aarch64elf.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/aarch64elf.xce";
      else
	return "ldscripts/aarch64elf.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/aarch64elf.xe";
      else
	return "ldscripts/aarch64elf.x";
    }
}
...
static bfd_boolean
gldaarch64elf_handle_option (int optc)
{
  switch (optc)
    {
    default:
	...
	einfo (_("%F%P: invalid --compress-debug-sections option: `%s'\n"),
      ...
      break;
    case OPTION_AUDIT:
	gldaarch64elf_append_to_separated_string (&audit, optarg);
	break;

    case 'P':
	gldaarch64elf_append_to_separated_string (&depaudit, optarg);
	break;
...
static void
gldaarch64elf_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_aarch64elf_emulation =
{
  gldaarch64elf_before_parse,
  syslib_default,
  hll_default,
  gldaarch64elf_after_parse,
  gldaarch64elf_after_open,
  after_check_relocs_default,
  gldaarch64elf_after_allocation,
  set_output_arch_default,
  ldemul_default_target,
  aarch64_elf_before_allocation,
  gldaarch64elf_get_script,
  "aarch64elf",
  "elf64-littleaarch64",
  gldaarch64elf_finish,
  aarch64_elf_create_output_section_statements,
  gldaarch64elf_open_dynamic_archive,
  gldaarch64elf_place_orphan,
  NULL,
  NULL,
  gldaarch64elf_add_options,
  gldaarch64elf_handle_option,
  NULL,
  gldaarch64elf_list_options,
  gldaarch64elf_load_symbols,
  NULL,
  NULL,
  NULL
};
