...
/* 32 bit ELF emulation code for armelfb
...
static void gldarmelfb_before_parse (void);
static void gldarmelfb_after_parse (void);
static void gldarmelfb_after_open (void);
static void gldarmelfb_before_allocation (void);
static void gldarmelfb_after_allocation (void);
static lang_output_section_statement_type *gldarmelfb_place_orphan
  (asection *, const char *, int);

static void
gldarmelfb_map_segments (bfd_boolean need_layout)
{
  int tries = 10;
...
static void
gldarmelfb_after_allocation (void)
{
  int ret;
  ...
  if (need_laying_out != -1)
    gldarmelfb_map_segments (need_laying_out);
}

static void
gldarmelfb_finish (void)
{
...
static bfd_boolean
gldarmelfb_load_symbols (lang_input_statement_type *entry)
{
  int link_class = 0;
...
static void
gldarmelfb_vercheck (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldarmelfb_stat_needed (lang_input_statement_type *s)
{
  struct stat st;
  const char *suffix;
  const char *soname;
...
static bfd_boolean
gldarmelfb_try_needed (struct dt_needed *needed,
				 int force)
{
  bfd *abfd;
  ...
  if (! force)
      ...
      if (needs != NULL)
	{
	  ...
	  lang_for_each_input_file (gldarmelfb_vercheck);
	  if (global_vercheck_failed)
	    {
	      bfd_close (abfd);
  ...
  global_found = NULL;
  lang_for_each_input_file (gldarmelfb_stat_needed);
  if (global_found != NULL)
    {
...
static bfd_boolean
gldarmelfb_search_needed (const char *path,
				    struct dt_needed *n, int force)
{
  ...
  if (name[0] == '/')
    return gldarmelfb_try_needed (n, force);
  ...
  while (1)
      ...
      while ((var = strchr (filename + offset, '$')) != NULL)
	{
      ...
      if (gldarmelfb_try_needed (&needed, force))
	return TRUE;
...
static void
gldarmelfb_check_needed (lang_input_statement_type *s)
{
  const char *soname;
...
gldarmelfb_after_open (void)
{
  ...
  for (l = needed; l != NULL; l = l->next)
	...
	continue;
      ...
      global_needed = l;
      global_found = NULL;
      lang_for_each_input_file (gldarmelfb_check_needed);
      if (global_found != NULL
	  && (bfd_elf_get_dyn_lib_class (global_found->the_bfd)
	      & DYN_AS_NEEDED) == 0)
	continue;
      ...
      if (global_found != NULL)
	{
	  ...
	  if (gldarmelfb_try_needed (&nn, TRUE))
	    continue;
	}
	 ...
	 their use.  See gldarmelfb_vercheck comment.  */
      for (force = 0; force < 2; force++)
	{
	  size_t len;
	  search_dirs_type *search;

	  if (gldarmelfb_search_needed (command_line.rpath_link,
						  &n, force))
	    break;
	  len = strlen (l->name);
	  for (search = search_head; search != NULL; search = search->next)
		...
		continue;
	      ...
	      nn.name = filename;
	      if (gldarmelfb_try_needed (&nn, force))
		break;
	      free (filename);
	    }
	  if (search != NULL)
...
static void
gldarmelfb_find_exp_assignment (etree_type *exp)
{
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldarmelfb_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldarmelfb_find_exp_assignment (exp->binary.lhs);
      gldarmelfb_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldarmelfb_find_exp_assignment (exp->trinary.cond);
      gldarmelfb_find_exp_assignment (exp->trinary.lhs);
      gldarmelfb_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldarmelfb_find_exp_assignment (exp->unary.child);
      break;
...
static void
gldarmelfb_find_statement_assignment (lang_statement_union_type *s)
{
  if (s->header.type == lang_assignment_statement_enum)
    gldarmelfb_find_exp_assignment (s->assignment_statement.exp);
}


/* used by before_allocation and handle_option. */
static void
gldarmelfb_append_to_separated_string (char **to, char *op_arg)
{
  if (*to == NULL)
    *to = xstrdup (op_arg);
  else
...
static void
gldarmelfb_before_allocation (void)
{
  const char *rpath;
  asection *sinterp;
  bfd *abfd;
  ...
  if (is_elf_hash_table (link_info.hash))
      ...
      if (!bfd_link_relocatable (&link_info))
	{
	 ...
	 referred to by dynamic objects.  */
      lang_for_each_statement (gldarmelfb_find_statement_assignment);
    }
	...
	if (audit_libs && *audit_libs != '\0')
	  {
		...
		if (cp != NULL && *cp != '\0')
		  gldarmelfb_append_to_separated_string (&depaudit, cp);
...
static lang_output_section_statement_type *
gldarmelfb_place_orphan (asection *s,
				   const char *secname,
				   int constraint)
{
...
static char *
gldarmelfb_get_script (int *isfile)
{
  *isfile = 1;

  if (bfd_link_relocatable (&link_info) && config.build_constructors)
    return "ldscripts/armelfb.xu";
  else if (bfd_link_relocatable (&link_info))
    return "ldscripts/armelfb.xr";
  else if (!config.text_read_only)
    return "ldscripts/armelfb.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/armelfb.xn";
	...
	return "ldscripts/armelfb.xdwe";
      else
	return "ldscripts/armelfb.xdw";
    }
  else if (bfd_link_pie (&link_info)
	   && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/armelfb.xdce";
      else
	return "ldscripts/armelfb.xdc";
    }
  else if (bfd_link_pie (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/armelfb.xde";
      else
	return "ldscripts/armelfb.xd";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc
	   && link_info.relro && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/armelfb.xswe";
      else
	return "ldscripts/armelfb.xsw";
    }
  else if (bfd_link_dll (&link_info) && link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/armelfb.xsce";
      else
	return "ldscripts/armelfb.xsc";
    }
  else if (bfd_link_dll (&link_info))
    {
      if (link_info.separate_code)
	return "ldscripts/armelfb.xse";
      else
	return "ldscripts/armelfb.xs";
    }
  else if (link_info.combreloc && link_info.relro
	   && (link_info.flags & DF_BIND_NOW))
    {
      if (link_info.separate_code)
	return "ldscripts/armelfb.xwe";
      else
	return "ldscripts/armelfb.xw";
    }
  else if (link_info.combreloc)
    {
      if (link_info.separate_code)
	return "ldscripts/armelfb.xce";
      else
	return "ldscripts/armelfb.xc";
    }
  else
    {
      if (link_info.separate_code)
	return "ldscripts/armelfb.xe";
      else
	return "ldscripts/armelfb.x";
    }
}
...
static bfd_boolean
gldarmelfb_handle_option (int optc)
{
  switch (optc)
    {
    default:
	...
	einfo (_("%F%P: invalid --compress-debug-sections option: `%s'\n"),
      ...
      break;
    case OPTION_AUDIT:
	gldarmelfb_append_to_separated_string (&audit, optarg);
	break;

    case 'P':
	gldarmelfb_append_to_separated_string (&depaudit, optarg);
	break;
...
static void
gldarmelfb_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_armelfb_emulation =
{
  gldarmelfb_before_parse,
  syslib_default,
  hll_default,
  gldarmelfb_after_parse,
  gldarmelfb_after_open,
  after_check_relocs_default,
  gldarmelfb_after_allocation,
  set_output_arch_default,
  ldemul_default_target,
  arm_elf_before_allocation,
  gldarmelfb_get_script,
  "armelfb",
  "elf32-bigarm",
  gldarmelfb_finish,
  arm_elf_create_output_section_statements,
  gldarmelfb_open_dynamic_archive,
  gldarmelfb_place_orphan,
  gldarmelfb_set_symbols,
  NULL,
  gldarmelfb_add_options,
  gldarmelfb_handle_option,
  NULL,
  gldarmelfb_list_options,
  gldarmelfb_load_symbols,
  NULL,
  NULL,
  NULL
};
