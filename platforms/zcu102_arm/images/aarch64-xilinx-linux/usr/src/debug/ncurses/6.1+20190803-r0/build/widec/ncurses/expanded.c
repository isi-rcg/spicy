...
{
 if (!((((*((SP)->_current_attr))).attr) == (((*c)).attr))) { attr_t chg = (((*((SP)->_current_attr))).attr); vidputs( (((*c)).attr), _nc_outch); if ((cur_term)->type. Numbers[4] > 0 && (((chg ^ (((*((SP)->_current_attr))).attr))) & SP->_xmc_suppress)) { ; _nc_do_xmc_glitch( chg); } };
}
