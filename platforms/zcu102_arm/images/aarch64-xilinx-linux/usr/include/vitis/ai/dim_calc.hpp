...
class DimCalc {
 public:
  DimCalc() = delete;
  DimCalc(const std::vector<size_t>& dims);
  DimCalc(const std::vector<size_t>& dims, const std::vector<size_t>& strides);
  ~DimCalc() = default;
  DimCalc(const DimCalc& other) = delete;
  DimCalc& operator=(const DimCalc& rhs) = delete;
