...
class DpuController {
 ...
 public:
  ...
  virtual uint32_t get_fingerprint(size_t device_core_id) const = 0;
  // device_core_id in [0, get_num_of_dpus());
  virtual void run(size_t device_core_idx, const uintptr_t code,
                   const std::vector<uintptr_t>& gen_reg) = 0;
};
}  // namespace xir
