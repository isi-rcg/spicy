...
class functionTracer(tracer.tracerBase.Tracer):
    def __init__(self):
        super().__init__('function', source=["ftrace"], compatible = {'machine': ["x86_64", "aarch64"]})
    
    def prepare(self, options: dict, debug: bool):
        """Process configure file & symbol table"""
        table = symbolTable(options.get('control').get('cmd')[0]).getSymbolTable()
        ...
        for section in traceListOri:
            ...
            for func in section.functions:
                traceList.append([func.name, func.libPath, "0x%x" % func.offset])
    ...
    def process(self, data):
        self.data = [l for l in data.get('ftrace', {}).get(self.name) if not l.startswith('#')]
