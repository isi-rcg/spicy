...
import os,re,csv,sys,string,time,argparse,json,signal,logging
...
def saityCheck(options:dict):
    ...
    if os.path.exists(xmodel) == False:
        logging.info("Xmodel not exist: [%s]" % xmodel)
...
def parseCfg(options:dict):
    ...
    """Timeout"""
    cmdline_timeout = options['cmdline_args'].get("timeout", 3)
    ...
    builtInList = vaitraceDefaults.biiltInFunctions
    enabledTraceList = traceCfg.get('enable_trace_list')
