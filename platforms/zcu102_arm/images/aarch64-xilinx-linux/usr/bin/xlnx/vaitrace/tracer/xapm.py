...
class APM:
    ...
    def start(self, interval=1.0):
        ...
        self.apmLib.apm_start(c_double(interval))
    ...
    def printData(self, data):
        ...
        read = [data.data[i]/self.interval/1024/1024 for i in range(0,10) if i % 2 == 0]
        info += "Read Ports:  "
        for d in read:
            info += "%8.1f" % d
        ...
        write = [data.data[i]/self.interval/1024/1024 for i in range(0,10) if i % 2 == 1]
        info += "Write Ports: "
        for d in write:
            info += "%8.1f" % d
    ...
    def transTimebase(self):
        for i in range(0, len(self.data)):
            for j in range(0, len(self.data[i].data)):
                self.data[i].data[j] = int(self.data[i].data[j] / self.interval)



class xapmTracer(tracer.tracerBase.Tracer):
    def __init__(self):
        super().__init__('xapm', source=[], compatible = {'machine': ["aarch64"]})
        self.apm = None
