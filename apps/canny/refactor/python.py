    ...
    for m in range(imr+10):
        for n in range(imc):
            ...
            if m > 6 and m < (imr+7): #7,8,9
                #np.float32()  np.sqrt()
                h:'int32' = np.float32(im3h[(m-7)%5][n])
                v:'int32' = np.float32(im3v[(m-7)%5][n])
                grad[(m-7)%5][n] = np.sqrt(h*h + v*v)
        ...
        if m > 9:
            for n in range(imc):
                ...
                if m-10 == 0 or m-10 == im.shape[0]-1 or n == 0 or n == im.shape[1] - 1:
                    continue
                #5/pi = 1.591549
                theta = (np.round(np.arctan2(im3v[(m-10)%5][n], im3h[(m-10)%5][n]) * 1.59) + 5)
                tq = (theta % 5) % 4
